...
class PageTree
  ...
  def initialize(depth, directory, parent_path, dir_to_title)
    @depth = depth
    @directory = directory
    @parent_path = parent_path
    @dir_to_title = dir_to_title
    @pages = SortedSet.new
    @subdirectories = SortedSet.new
  end

  def directory_title
    @dir_to_title[directory_path]
  end
  ...
  def add_file_path_list(source_page, dir_segments)
    ...
    new_subdirectory = PageTree.new(@depth + 1, first, directory_path, @dir_to_title)
...
def prepare_page_tree(context)
  ...
  root = PageTree.new(0, "", "", dir_to_title)
